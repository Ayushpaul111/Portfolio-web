  ...
  },
  {
    ...
    content: (
      <div className="bg-green-700 p-2 sm:p-4 rounded-xl font-bold text-sm sm:text-xl md:text-lg text-gray-100 hover:scale-105 focus:scale-105 transition-transform duration-500 ease-in-out hover:cursor-pointer">
        ...
        <p className="mt-2">I'm not a developer im a bulder.</p>
      </div>
    ),
  },
  {
...
const GalleryGrid = () => {
  // Sample data - replace with your own images/videos

  const renderItem = (item: any, index: number) => {
    const baseClasses =
      "w-full rounded-xl shadow hover:scale-105 focus:scale-105 transition-transform duration-500 ease-in-out";
  ...
  return (
    <div
      className="grid grid-cols-2 md:grid-cols-3 gap-4 [&_img]:hover:cursor-pointer"
      style={{ opacity: 1 }}
    >
      {/* First column */}
      <div className="grid gap-4" style={{ opacity: 1 }}>
      ...
      <div
        className="grid grid-cols-2 md:grid-cols-none col-span-2 md:col-auto gap-4"
        style={{ opacity: 1 }}
      >
